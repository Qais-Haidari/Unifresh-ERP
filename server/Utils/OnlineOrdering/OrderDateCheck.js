const OrderDateCheck = async (username) => {
    let Report = `
    
    select DATESBETWEEN.OUTPUTDATE, SCHED.DISPATCHDAY from 
(
  select 
  EXTRACT(Weekday from RETURN_DATESBETWEEN.OUTPUTDATE) as "OUTDAY", 
  RETURN_DATESBETWEEN.* 
  from 
  RETURN_DATESBETWEEN(current_date + 1,dateadd(10 day to dateadd(1 month to current_date))) 
) DATESBETWEEN 
left outer join
(
select DISTINCT 
...
left outer join 
(
select salesinvoiceheader.* from salesinvoiceheader join salesinvoicelines on salesinvoiceheader.invoicenumber = salesinvoicelines.invoicenumber and salesinvoicelines.linecode <> 'GIFT1'
where 
salesinvoiceheader.customer = '${username}' and 
salesinvoiceheader.invoicedate >= current_date - 5 and 
salesinvoiceheader.invoicedate <= dateadd(10 day to dateadd(1 month to current_date)) 
...
) SCHED on OUTPUTDATE = DISPATCHDAY 
where 
SCHED.DISPATCHDAY is null
order by 
OUTPUTDATE asc
